...
class RayPPOTrainer:
    ...
    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns):
        ...
        metric_dict = {}
        benchmark_sources = {str(src) for src in data_sources if str(src).startswith("agent_guard_benchmark/")}

        def add_benchmark_f1(data_source):
            indices = [idx for idx, src in enumerate(data_sources) if str(src) == data_source]
            tp = sum(float(reward_extra_infos_dict.get("unsafe_tp", [0.0] * len(data_sources))[idx] or 0.0) for idx in indices)
            fp = sum(float(reward_extra_infos_dict.get("unsafe_fp", [0.0] * len(data_sources))[idx] or 0.0) for idx in indices)
            fn = sum(float(reward_extra_infos_dict.get("unsafe_fn", [0.0] * len(data_sources))[idx] or 0.0) for idx in indices)
            precision = tp / max(1.0, tp + fp)
            recall = tp / max(1.0, tp + fn)
            f1 = 0.0 if precision + recall == 0.0 else 2.0 * precision * recall / (precision + recall)
            metric_dict[f"val-core/{data_source}/unsafe_f1"] = f1

        for data_source in sorted(benchmark_sources):
            add_benchmark_f1(data_source)

        for data_source, var2metric2val in data_src2var2metric2val.items():
            if str(data_source).startswith("agent_guard_benchmark/"):
                continue
