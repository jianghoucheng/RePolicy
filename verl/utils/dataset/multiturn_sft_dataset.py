...
def print_assembled_message(tokenizer, message_list, input_ids, loss_mask, attn_mask, tools):
    ...
    try:
        ...
        logger.warning(f"print_assembled_message failed (non-fatal): {type(e).__name__}: {e}")


def _message_for_template(message: dict[str, Any]) -> dict[str, Any]:
    """Drop local training-only fields before applying the tokenizer chat template."""

    return {k: v for k, v in message.items() if k not in {"loss", "train_on_response"}}
...
class MultiTurnSFTDataset(Dataset):
    ...
    def _read_files_and_process(self):
        ...
        total = len(self.rows)
        print(f"dataset len: {len(self.rows)}")
        ...
        if self.max_samples > 0 and self.max_samples < total:
            ...
            self.rows = [self.rows[i] for i in indices.tolist()]
            print(f"selected {self.max_samples} random samples out of {total}")

        # Extract messages list from dataframe
        self.messages = [convert_nested_value_to_list_recursive(row[self.messages_key]) for row in self.rows]

        # Extract tools list from dataframe
        if self.rows and self.tools_key in self.rows[0]:
            self.tools = [convert_nested_value_to_list_recursive(row[self.tools_key]) for row in self.rows]
        else:
            self.tools = None
        # Extract enable_thinking list from dataframe
        if self.rows and self.enable_thinking_key in self.rows[0]:
            self.enable_thinking = [row.get(self.enable_thinking_key) for row in self.rows]
        else:
    ...
    def _process_single_message(
        ...
        template_message = _message_for_template(message)
        ...
        train_on_message = message.get("loss", message.get("train_on_response", None))
        if train_on_message is None:
            train_on_message = role == "assistant"
        else:
            train_on_message = bool(train_on_message)

        if role == "assistant" and train_on_message:
    ...
    def __getitem__(self, item):
        ...
        print_assembled_message(self.tokenizer, template_messages, input_ids, loss_mask, attention_mask, tools)
        try:
            self.sanity_check(input_ids, template_messages, tools, enable_thinking)
        except Exception as e:
            logger.warning_once(f"sanity_check failed (non-fatal): {type(e).__name__}: {e}")
