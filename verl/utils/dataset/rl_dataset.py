...
class RLHFDataset(Dataset):
    ...
    def _safe_concatenate(self, dataframes):
        """Concatenate datasets with potentially mismatched schemas by keeping only common columns."""
        # Find common columns across all datasets
        common_cols = set(dataframes[0].column_names)
        for df in dataframes[1:]:
            common_cols &= set(df.column_names)
        # Keep only common columns and cast all to string for mismatched types
        aligned = []
        for df in dataframes:
            aligned.append(df.select_columns(list(common_cols)))
        try:
            return datasets.concatenate_datasets(aligned)
        except ValueError:
            # If still fails due to type mismatch, use the first dataset's features as reference
            from datasets import Features, Value
            ref_features = aligned[0].features
            casted = [aligned[0]]
            for df in aligned[1:]:
                try:
                    casted.append(df.cast(ref_features))
                except Exception:
                    # Last resort: only keep columns that can be cast
                    safe_cols = []
                    for col in common_cols:
                        try:
                            df.cast_column(col, ref_features[col])
                            safe_cols.append(col)
                        except Exception:
                            pass
                    if safe_cols:
                        casted.append(df.select_columns(safe_cols).cast(Features({c: ref_features[c] for c in safe_cols})))
            return datasets.concatenate_datasets(casted)
    ...
    def _read_files_and_tokenize(self):
        ...
        self.dataframe: datasets.Dataset = datasets.concatenate_datasets(dataframes) if len(dataframes) == 1 else self._safe_concatenate(dataframes)
