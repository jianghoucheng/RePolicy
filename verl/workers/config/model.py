...
class HFModelConfig(BaseConfig):
    ...
    def __post_init__(self):
        ...
        if self.load_tokenizer:
            ...
            # For text-only models (e.g. Qwen3.5-4B) that incorrectly return a VL processor,
            # check if the model config indicates it's actually a text-only model and force processor=None
            if self.processor is not None:
                try:
                    config = AutoConfig.from_pretrained(self.local_tokenizer_path, trust_remote_code=self.trust_remote_code)
                    model_type = getattr(config, "model_type", "")
                    # If model_type doesn't contain 'vl' or 'vision', it's text-only
                    if "vl" not in model_type.lower() and "vision" not in model_type.lower():
                        self.processor = None
                except Exception:
                    pass
        ...
        if self.custom_chat_template is not None:
            # Support loading chat template from a .jinja file path
            chat_template_str = self.custom_chat_template
            if chat_template_str.endswith(".jinja") and os.path.isfile(chat_template_str):
                with open(chat_template_str, "r") as f:
                    chat_template_str = f.read()
            # Always override tokenizer's chat_template
            self.tokenizer.chat_template = chat_template_str
            if self.processor is not None:
                self.processor.chat_template = chat_template_str
